...
import getHealthProjectWithParticipants from "../../queries/getHealthProjectWithParticipants"
...
export default function HealthProjectViewPage() {
  const { id } = useParams<{ id: string }>()
  const [data] = useQuery(getHealthProjectWithParticipants, { id: Number(id) })
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <div className="flex flex-col items-end gap-2">
              <span className={`text-xs px-3 py-1 rounded-full font-semibold ${new Date(project.endDate) >= new Date() ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}>
                {new Date(project.endDate) >= new Date() ? "Active" : "Ended"}
              </span>
          ...
          <div className="mt-4 flex flex-wrap gap-1.5">
            {project.healthStatuses.map((s) => (
              <Chip
                ...
                label={s.statusName}
                size="small"
                sx={{ backgroundColor: statusColor[s.statusName] ?? "#94a3b8", color: "#fff", fontSize: "0.7rem" }}
              />
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          {participants.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-2">
              ...
              <p className="text-base font-medium">No participants found</p>
              <p className="text-sm">No residents match this project's target health statuses yet.</p>
            </div>
          ) : (
              ...
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 text-slate-500 text-xs uppercase">
                  <tr>
                    ...
                    <th className="px-5 py-3 text-left">Last Checkup</th>
                  </tr>
                </thead>
                  ...
                  {participants.map((r) => {
                    ...
                    return (
                      <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-5 py-3 font-semibold text-slate-700">{latest?.bmi ?? "—"}</td>
                        ...
                        <td className="px-5 py-3 text-slate-500 whitespace-nowrap">
                          {latest
                            ? new Date(latest.dateOfCheckup).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
                            : "—"}
                        </td>
                      </tr>
                    )
